...
public class BinaryTreeAlgorithms {
    ...
    public static <T> Iterable<BinaryTree.TreeNode<T>> byLevelNodes(BinaryTree.TreeNode<T> treeNode) {
        return () -> {
            Queue<BinaryTree.TreeNode<T>> queue = new LinkedList<>();
            queue.add(treeNode);

            return new Iterator<BinaryTree.TreeNode<T>>() {
                @Override
                public boolean hasNext() {
                    return queue.size() > 0;
                }
    ...
    public static <T> LinkedList<BinaryTree.TreeNode<T>> byLevelNodesList(BinaryTree.TreeNode<T> treeNode, boolean isReversed) {
        LinkedList<BinaryTree.TreeNode<T>> nodes = new LinkedList<>();
        for (BinaryTree.TreeNode<T> node : BinaryTreeAlgorithms.byLevelNodes(treeNode)) {
            nodes.add(node);
        }
        if (isReversed) {
